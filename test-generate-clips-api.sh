#!/bin/bash

# Script para testar generate-clips usando apenas APIs REST
set -e

echo "🎬 Teste Generate-Clips (API Only)"
echo "==================================="

# Configurações
SUPABASE_URL="http://127.0.0.1:54321"
SERVICE_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImV4cCI6MTk4MzgxMjk5Nn0.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU"

# IDs fixos para facilitar debug
USER_ID="11111111-1111-1111-1111-111111111111"
VIDEO_ID="22222222-2222-2222-2222-222222222222"
ANALYSIS_ID="33333333-3333-3333-3333-333333333333"

echo "User ID: $USER_ID"
echo "Video ID: $VIDEO_ID"
echo ""

# 1. Limpar dados anteriores
echo "📝 1. Limpando dados anteriores..."
curl -s -X DELETE "$SUPABASE_URL/rest/v1/clips?video_id=eq.$VIDEO_ID" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" > /dev/null

curl -s -X DELETE "$SUPABASE_URL/rest/v1/content_analysis?video_id=eq.$VIDEO_ID" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" > /dev/null

curl -s -X DELETE "$SUPABASE_URL/rest/v1/videos?id=eq.$VIDEO_ID" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" > /dev/null

curl -s -X DELETE "$SUPABASE_URL/rest/v1/profiles?id=eq.$USER_ID" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" > /dev/null

echo "✅ Dados limpos"

# 2. Criar usuário via auth admin
echo "📝 2. Criando usuário via auth admin..."
AUTH_RESPONSE=$(curl -s -X POST "$SUPABASE_URL/auth/v1/admin/users" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" \
  -H "Content-Type: application/json" \
  -d "{
    \"email\": \"test@clipsforge.com\",
    \"password\": \"password123\",
    \"email_confirm\": true,
    \"user_metadata\": {
      \"full_name\": \"Test User\"
    }
  }")

echo "Auth response: $AUTH_RESPONSE"
echo "✅ Usuário auth criado"

# 3. Criar profile (usando ID fixo)
echo "📝 3. Criando profile..."
PROFILE_RESPONSE=$(curl -s -X POST "$SUPABASE_URL/rest/v1/profiles" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" \
  -H "Content-Type: application/json" \
  -H "Prefer: return=minimal" \
  -d "{
    \"id\": \"$USER_ID\",
    \"email\": \"test@clipsforge.com\",
    \"full_name\": \"Test User\",
    \"plan_type\": \"pro\"
  }")

if [[ "$PROFILE_RESPONSE" == *"error"* ]]; then
  echo "❌ Erro ao criar profile: $PROFILE_RESPONSE"
  echo "Tentando usar usuário existente..."
else
  echo "✅ Profile criado"
fi

# 4. Criar vídeo
echo "📝 4. Criando vídeo..."
VIDEO_RESPONSE=$(curl -s -X POST "$SUPABASE_URL/rest/v1/videos" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" \
  -H "Content-Type: application/json" \
  -H "Prefer: return=minimal" \
  -d "{
    \"id\": \"$VIDEO_ID\",
    \"user_id\": \"$USER_ID\",
    \"title\": \"Vídeo de Teste\",
    \"description\": \"Teste para generate-clips\",
    \"original_filename\": \"test.mp4\",
    \"file_size_bytes\": 1024,
    \"duration_seconds\": 60,
    \"processing_status\": \"ready\",
    \"cloudinary_public_id\": \"test_video\",
    \"cloudinary_secure_url\": \"https://res.cloudinary.com/test/video/upload/test_video.mp4\"
  }")

if [[ "$VIDEO_RESPONSE" == *"error"* ]]; then
  echo "❌ Erro ao criar vídeo: $VIDEO_RESPONSE"
  exit 1
fi

echo "✅ Vídeo criado"

# 5. Criar análise de conteúdo
echo "📝 5. Criando análise de conteúdo..."
ANALYSIS_RESPONSE=$(curl -s -X POST "$SUPABASE_URL/rest/v1/content_analysis" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" \
  -H "Content-Type: application/json" \
  -H "Prefer: return=minimal" \
  -d "{
    \"id\": \"$ANALYSIS_ID\",
    \"video_id\": \"$VIDEO_ID\",
    \"user_id\": \"$USER_ID\",
    \"clips_suggestions\": [
      {
        \"start_time\": 5,
        \"end_time\": 35,
        \"duration\": 30,
        \"title\": \"Clip Viral Incrível\",
        \"description\": \"Um momento épico do vídeo\",
        \"viral_score\": 8.5,
        \"hook_strength\": 9.2,
        \"best_platforms\": [\"tiktok\", \"instagram\", \"youtube\"],
        \"content_category\": \"entretenimento\",
        \"hashtags\": [\"#viral\", \"#incrivel\", \"#clips\"]
      },
      {
        \"start_time\": 20,
        \"end_time\": 50,
        \"duration\": 30,
        \"title\": \"Momento Épico\",
        \"description\": \"Sequência incrível\",
        \"viral_score\": 7.8,
        \"hook_strength\": 8.1,
        \"best_platforms\": [\"instagram\", \"youtube\", \"tiktok\"],
        \"content_category\": \"entretenimento\",
        \"hashtags\": [\"#epico\", \"#viral\", \"#momento\"]
      }
    ],
    \"analysis_completed\": true
  }")

if [[ "$ANALYSIS_RESPONSE" == *"error"* ]]; then
  echo "❌ Erro ao criar análise: $ANALYSIS_RESPONSE"
  exit 1
fi

echo "✅ Análise criada"

# 6. Verificar dados antes do teste
echo "📝 6. Verificando dados..."
echo "Vídeos no banco:"
curl -s -X GET "$SUPABASE_URL/rest/v1/videos?select=id,title,user_id,processing_status" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" | jq . 2>/dev/null || echo "Erro ao buscar vídeos"

echo "Análises no banco:"
curl -s -X GET "$SUPABASE_URL/rest/v1/content_analysis?select=id,video_id,analysis_completed" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY" | jq . 2>/dev/null || echo "Erro ao buscar análises"

# 7. Testar generate-clips
echo "📝 7. Testando generate-clips..."
echo "Chamando função generate-clips..."

RESPONSE=$(curl -s -X POST "$SUPABASE_URL/functions/v1/generate-clips" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"video_id\": \"$VIDEO_ID\"}")

echo "📄 Resposta da função:"
echo "$RESPONSE" | jq . 2>/dev/null || echo "$RESPONSE"

# 8. Verificar clips criados
echo "📝 8. Verificando clips no banco..."
CLIPS=$(curl -s -X GET "$SUPABASE_URL/rest/v1/clips?video_id=eq.$VIDEO_ID" \
  -H "Authorization: Bearer $SERVICE_KEY" \
  -H "apikey: $SERVICE_KEY")

CLIPS_COUNT=$(echo "$CLIPS" | jq '. | length' 2>/dev/null || echo "0")
echo "📊 Clips encontrados: $CLIPS_COUNT"

if [ "$CLIPS_COUNT" -gt 0 ]; then
  echo "🎉 SUCESSO! Clips foram gerados:"
  echo "$CLIPS" | jq -r '.[] | "• " + .title + " (" + (.duration_seconds|tostring) + "s) - Score: " + (.ai_viral_score|tostring)' 2>/dev/null || echo "$CLIPS"
else
  echo "❌ FALHA: Nenhum clip foi gerado"
  echo "Detalhes dos clips:"
  echo "$CLIPS"
fi

echo ""
echo "🔍 IDs para debug:"
echo "User ID: $USER_ID"
echo "Video ID: $VIDEO_ID"
echo "Analysis ID: $ANALYSIS_ID" 